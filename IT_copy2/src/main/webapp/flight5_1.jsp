<%@page import="dto.Fli_seat_two_DTO"%>
...
int fli_seat_two_num = (Integer)request.getAttribute("fli_seat_two_num");
/* flight2test 값  */
...
<form action="flightAction6.jsp" method="post" id="flightForm">
<!-- 운임 & 수화물  -->
...
<%
    ...
    Vector<Fli_seat_two_DTO> listseat2 = fd.seat2();
    int columnCount = 0; // 열 카운터 변수 추가
    for (int i = 0; i < listseat2.size(); i++) {
    	Fli_seat_two_DTO fst = listseat2.get(i);
        %>
        <div class="divwrap4_1">
        <input type="hidden" id="seatNumber2" name="seatNumber2">
            <div class="divwrap4_2 clicked1" onclick="handleClick(
                        '<%= selectedPrice2 %>',
            '<%= price %>',
    '<%= fst.getFli_seat_two_num() %>',
    '<%= seatNumber %>',
    '<%= air_deppt1 %>',
    '<%= air_arrpt1 %>',
    '<%= air_dday1 %>',
...
function handleClick(selectedPrice2,price,seatNumber2,seatNumber, air_deppt1, air_arrpt1, air_dday1, air_aday1, air_model1, air_fligname1, air_dday_time1, air_aday_time1, air_time_minute1,
		 air_deppt, air_arrpt, air_dday, air_aday, air_model, air_fligname, air_dday_time, air_aday_time, air_time_minute, sum) {
		  ...
		  if (this.classList.contains('clicked1')) {
		    ...
		    var seatNumber2 = this.innerText.trim();
		    resultElement.innerText = seatNumber2;
		    var seatnumber2input = document.getElementById('seatNumber2');
		    seatnumber2input.value = seatNumber2;
		    ...
		    console.log("좌석 번호2");
		    console.log(seatNumber2);
			
		    var hiddenInput1_1 = document.getElementById('seatNumber').value;
		    hiddenInput1_1.value = seatNumber;
		    console.log("좌석 번호1");
		...
		document.addEventListener('DOMContentLoaded', function() {
		  ...
		  divwrap4_2s.forEach(function(divwrap4_2) {
		    divwrap4_2.addEventListener('click', function() {
		      ...
		      const price = '<%= price%>';
		      const seatNumber2 = '<%= fst.getFli_seat_two_num() %>';
		      ...
		      handleClick.call(this, selectedPrice2,price, seatNumber2, seatNumber, air_deppt1, air_arrpt1, air_dday1, air_aday1, air_model1, air_fligname1, air_dday_time1, air_aday_time1, air_time_minute1, air_deppt, air_arrpt, air_dday, air_aday, air_model, air_fligname, air_dday_time, air_aday_time, air_time_minute, sum);
		    });
		  });
		});
</script>
