...
<div >
<form action="flightAction3.jsp" id="flightForm" method="post">
...
for(int i = 0; i < list1.size(); i++){
	...
	if(ai.getAir_deppt().equals(air_deppt) && ai.getAir_arrpt().equals(air_arrpt) && ai.getAir_dday().equals(air_dday)) {
...
<div class="divwrap3" onclick="updateAirPrice('<%= ai.getAir_price() %>', '<%= air_deppt %>', '<%= air_arrpt %>', '<%= air_dday %>', '<%= air_aday %>', '<%= ai.getAir_model() %>', '<%= ai.getAir_fligname() %>', '<%= ai.getAir_dday_time() %>', '<%= ai.getAir_aday_time() %>', '<%= ai.getAir_time_minute() %>')">
<div class="divwrap4">
...
<input type="hidden" id="selected_price" name="selected_price" value="<%= ai.getAir_price() %>" />
</div>
...
<input type="submit" class="divbottom2_3_1" value="다음 여정" onclick="submitFlightForm()">
